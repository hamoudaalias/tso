"""
TSO v3 — Procgen Heist (version rapide).
Démontre :
1. DualLIF → features invariantes malgré textures changeantes
2. Memorisation one-shot de la sortie via AssociativeMemory/Cerebellum
3. Adaptation rapide sans fine-tuning

NORMAL (entraîné) vs AMNÉSIQUE (reset per episode) vs ALEA.
"""
...
def train_v1(env_name="heist", n_steps=200, n_protos=16):
    ...
    for step in range(max_steps):
        ...
        if train:
            ...
            if reward > 0:
                ...
                v1_nv = encode_v1(v1, next_obs)
                mem.store(v1_nv, action)
...
def main():
    ...
    v1 = train_v1("heist", n_steps=200, n_protos=16)
    ...
    te = gym.make("procgen-heist-v0", start_level=100, num_levels=50)
    wn, rn, _ = 0, [], None
    ...
    te2 = gym.make("procgen-heist-v0", start_level=100, num_levels=50)
    wa, ra = 0, []
    for ep in range(n_eval):
    ...
    te3 = gym.make("procgen-heist-v0", start_level=100, num_levels=50)
    wr, rr = 0, []
    for ep in range(n_eval):
